package io.github.samanos.mqtt
...
object Producer {
  ...
  def flow(topic: String, conf: Config) = {
...
object Consumer {
  ...
  def source(topic: String, conf: Config) = {
    Source.queue[MqttMessage](32, OverflowStrategy.dropNew).mapMaterializedValue { queue =>
      val opts = new MqttConnectOptions()
      opts.setCleanSession(true)
      opts.setSocketFactory(getSslSocketFactory(conf.getString("ca-crt"), conf.getString("cl-crt"), conf.getString("private-key")))
      ...
      client.setCallback(new MqttCallback {
        ...
        def messageArrived(topicArrived: String, message: MqttMessage) =
          if (topicArrived == topic) queue.offer(message)
